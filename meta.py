...
class Meta:
  ...
  def __init__(self):
    if path.exists('./models/openai_clip'):
      ...
      self._config = {
        'clip_model':  CLIPConfig.from_pretrained('/Volumes/LLMs/clip-ViT-B-32/0_CLIPModel').to_dict(),
      }
      try:
        # try as if it was a regular hf model
        self._config['text_model'] = AutoConfig.from_pretrained('/Volumes/LLMs/clip-ViT-B-32-multilingual-v1').to_dict()
      except (RuntimeError, TypeError, NameError, Exception, EnvironmentError):
        # now try as if it's a ST CLIP model
        self._config['text_model'] = AutoConfig.from_pretrained('/Volumes/LLMs/clip-ViT-B-32/0_CLIPModel').to_dict()
